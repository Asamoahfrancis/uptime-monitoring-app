...
exports.handler = {
    ...
    users: (data, userCallback) => { },
    tokens: (data, tokenCallback) => { },
    checks: (data, checksCallback) => { },
};
const verifiedToken = (id, phone, callback) => {
    data_1.lib.read('tokens', id, (error, _data) => {
        if (!error && _data) {
...
//users
exports.handler.users = (data, userCallback) => {
    const acceptableMethods = ['POST', 'GET', 'PUT', 'DELETE'];
    if (acceptableMethods.indexOf(data.method) > -1) {
        exports.handler._users[data.method](data, (statusCode, payload) => {
...
exports.handler.checks = (data, checksCallback) => {
    const acceptableMethods = ['POST', 'GET', 'PUT', 'DELETE'];
    if (acceptableMethods.indexOf(data.method) > -1) {
        exports.handler._checks[data.method](data, (statusCode, payload) => {
...
exports.handler._users.POST = (data, postCallback) => {
    ...
    if (firstName && lastName && phone && password && tosAgreement) {
        data_1.lib.read('users', phone, (error, data) => {
            if (error) {
                const hashedPassword = helpers_1.helpers.hash(password);
                const userObj = {
                    firstName: firstName,
                    lastName: lastName,
                    phone: phone,
                ...
                data_1.lib.create('users', phone, userObj, (error) => {
                    if (!error) {
...
exports.handler._users.GET = (data, getCallback) => {
    ...
    if (phone) {
        ...
        if (token) {
            verifiedToken(token, phone, (error) => {
                if (error) {
                    data_1.lib.read('users', phone, (error, _data) => {
                        if (!error && _data) {
...
exports.handler._users.PUT = (data, putCallback) => {
    const firstName = typeof (data.payload.firstName) === 'string' && data.payload.firstName.trim().length > 0 ? data.payload.firstName.trim() : false;
    const lastName = typeof (data.payload.lastName) === 'string' && data.payload.lastName.trim().length > 0 ? data.payload.lastName.trim() : false;
    const phone = typeof (data.payload.phone) === 'string' && data.payload.phone.trim().length > 0 ? data.payload.phone.trim() : false;
    const password = typeof (data.payload.password) === 'string' && data.payload.password.trim().length > 0 ? data.payload.password.trim() : false;
                    ...
                    if (firstName || lastName || password) {
                        data_1.lib.read('users', phone, (error, _data) => {
                            if (!error && _data) {
                                ...
                                if (password) {
                                    _data.hashedPassword = helpers_1.helpers.hash(password);
                                }
                                data_1.lib.update('users', phone, _data, (resp) => {
                                    if (!resp) {
...
exports.handler._users.DELETE = (data, deleteCallback) => {
    const phone = typeof (data.queryStringObj.phone) === 'string' && data.queryStringObj.phone.trim().length > 0 ? data.queryStringObj.phone.trim() : false;
    if (phone) {
        const token = typeof (data.headers.token) == 'string' ? data.headers.token : false;
        if (token) {
            verifiedToken(token, phone, (error) => {
                if (error) {
                    data_1.lib.read('users', phone, (error, _data) => {
                        if (!error && _data) {
                            data_1.lib.delete('users', phone, (error) => {
                                if (!error) {
...
//tokens
exports.handler.tokens = (data, tokensCallback) => {
    const acceptableMethods = ['POST', 'GET', 'PUT', 'DELETE'];
    if (acceptableMethods.indexOf(data.method) > -1) {
        exports.handler._tokens[data.method](data, (statusCode, payload) => {
...
exports.handler._tokens.POST = (data, posttokenCallback) => {
    const phone = typeof (data.payload.phone) === 'string' && data.payload.phone.trim().length > 0 ? data.payload.phone.trim() : false;
    const password = typeof (data.payload.password) === 'string' && data.payload.password.trim().length > 0 ? data.payload.password.trim() : false;
    if (phone && password) {
        data_1.lib.read('users', phone, (error, _data) => {
            if (!error && data) {
                ...
                if (hashedPassword == _data.hashedPassword) {
                    const tokenId = helpers_1.helpers.createRandomString(20);
                    ...
                    data_1.lib.create('tokens', tokenId, tokenObj, (error) => {
                        if (!error) {
...
exports.handler._tokens.GET = (data, gettokenCallback) => {
...
exports.handler._tokens.PUT = (data, puttokenCallback) => {
        ...
        data_1.lib.read('tokens', id, (error, _data) => {
            if (!error && _data) {
                if (_data.expires > Date.now()) {
                    ...
                    data_1.lib.update('tokens', id, _data, (resp) => {
                        if (!resp) {
...
exports.handler._tokens.DELETE = (data, deletetokenCallback) => {
    const id = typeof (data.queryStringObj.id) === 'string' && data.queryStringObj.id.trim().length > 0 ? data.queryStringObj.id.trim() : false;
    if (id) {
        data_1.lib.read('tokens', id, (error, _data) => {
            if (!error && _data) {
                data_1.lib.delete('tokens', id, (error) => {
                    if (!error) {
...
//checks
exports.handler._checks.POST = (data, postcheckCallback) => {
    ...
    if (protocol && url && methods && successCodes && timeoutSeconds) {
        const token = typeof (data.headers.token) === 'string' ? data.headers.token : false;
        if (token) {
            data_1.lib.read('tokens', token, (error, _data) => {
                if (!error && _data) {
                    const userPhone = _data.phone;
                    data_1.lib.read('users', userPhone, (error, _data_) => {
                        if (!error && _data_) {
                            const userCheck = typeof (_data_.checks) === 'object' && _data_.checks instanceof Array ? _data_.checks : [];
                            if (userCheck.length <= config_1.environmentToExport.maxChecks) {
                                const checkId = helpers_1.helpers.createRandomString(20);
                                const checkObj = {
                                    'id': checkId,
                                    'userPhone': userPhone,
                                    'protocol': protocol,
                                ...
                                data_1.lib.create('checks', checkId, checkObj, (error) => {
                                    if (!error) {
                                        _data_.checks = userCheck;
                                        _data_.checks.push(checkId);
                                        data_1.lib.update('users', userPhone, _data_, (error) => {
                                            if (!error) {
...
exports.handler._checks.GET = (data, getcheckCallback) => {
    const id = typeof (data.queryStringObj.id) === 'string' && data.queryStringObj.id.trim().length > 0 ? data.queryStringObj.id.trim() : false;
    if (id) {
        data_1.lib.read('checks', id, (error, checkdata) => {
            if (!error && checkdata) {
...
exports.handler._checks.PUT = (data, putcheckcallback) => {
    const protocol = typeof (data.payload.protocol) === 'string' && ['https', 'http'].indexOf(data.payload.protocol) > -1 ? data.payload.protocol : false;
    const url = typeof (data.payload.url) === 'string' && data.payload.url.trim().length > 0 ? data.payload.url.trim() : false;
    const methods = typeof (data.payload.methods) === 'string' && ['POST', 'GET', 'PUT', 'DELETE'].indexOf(data.payload.methods) > -1 ? data.payload.methods : false;
    const successCodes = typeof (data.payload.successCodes) === 'object' && data.payload.successCodes instanceof Array && data.payload.successCodes.length > 0 ? data.payload.successCodes : false;
                ...
                if (!error && checkdata) {
                    ...
                    verifiedToken(token, checkdata.userPhone, (res) => {
                        if (res) {
                            ...
                            data_1.lib.update('checks', id, checkdata, (error) => {
                                if (!error) {
...
exports.handler._checks.DELETE = (data, deletecheckCallback) => {
    const id = typeof (data.queryStringObj.id) === 'string' && data.queryStringObj.id.trim().length > 0 ? data.queryStringObj.id.trim() : false;
    if (id) {
        data_1.lib.read('checks', id, (error, checkdata) => {
            if (!error && checkdata) {
                ...
                verifiedToken(token, checkdata.userPhone, (mess) => {
                    if (mess) {
                        data_1.lib.delete('checks', id, (error) => {
                            if (!error) {
                                data_1.lib.read('users', checkdata.userPhone, (error, userData) => {
                                    ...
                                    if (!error) {
                                        ...
                                        if (checkposition > -1) {
                                            ...
                                            data_1.lib.update('users', userData.phone, userData, (error) => {
                                                if (!error) {
...
exports.handler.ping = (data, callback) => {
    callback(200);
};
exports.handler.notFound = (data, callback) => {
    callback(404);
};
